...
use drillx::Solution;
use ze_api::prelude::*;
use ze_boost_api::{consts::BOOST_DENOMINATOR, state::{Boost, Reservation}};
#[allow(deprecated)]
use solana_program::{
    keccak::hashv,
...
pub fn process_mine(accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    let bus = bus_info.is_bus()?.as_account_mut::<Bus>(&ze_api::ID)?;
    let config = config_info
        .is_config()?
        .as_account::<Config>(&ze_api::ID)?
        .assert_err(
            |c| c.last_reset_at.saturating_add(EPOCH_DURATION) > t,
            ZeError::NeedsReset.into(),
        )?;
    let proof = proof_info
        .as_account_mut::<Proof>(&ze_api::ID)?
        .assert_mut_err(
    ...
    if !solution.is_valid(&proof.challenge) {
        return Err(ZeError::HashInvalid.into());
    }
    ...
    if difficulty.lt(&(config.min_difficulty as u32)) {
        return Err(ZeError::HashTooEasy.into());
    }
    ...
    if let [boost_info, _boost_proof_info, reservation_info] = optional_accounts {
        // Load boost accounts.
        let boost = boost_info.as_account::<Boost>(&ze_boost_api::ID)?;
        reservation_info
            .as_account::<Reservation>(&ze_boost_api::ID)?
    ...
    // Apply bus limit.
    //
    // Busses are limited to distributing 1 ZE per epoch. The payout amount must be capped to whatever is
    // left in the selected bus. This limits the maximum amount that will be paid out for any given hash to 1 ZE.
    let net_reward = gross_penalized_reward.min(bus.rewards).min(ONE_ZE);
...
fn authenticate(data: &[u8], proof_address: &Pubkey) -> ProgramResult {
    if let Ok(Some(auth_address)) = parse_auth_address(data) {
        if proof_address.ne(&auth_address) {
            return Err(ZeError::AuthFailed.into());
        }
    } else {
        return Err(eError::AuthFailed.into());
    }
    Ok(())
}
