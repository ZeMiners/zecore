...
/// Reset tops up the bus balances, updates the base reward rate, and sets up the ZE program for the next epoch.
pub fn process_reset(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    ...
    let config = config_info
        .is_config()?
        .as_account_mut::<Config>(&ze_api::ID)?;
    let mint = mint_info
    ...
    if mint.supply.ge(&MAX_SUPPLY) {
        return Err(ZeError::MaxSupply.into());
    }
...
///
/// The new rate is then smoothed by a constant factor to avoid large fluctuations. In Ze's case,
